...
const checkName = (type, name) => {
    if (!Collector._isFormattedName(name)) {
        throw new Error(getNameErrMsg(type));
    }
};
...
class Collector extends Picker {
    constructor ({server, client, consumer, provider, brokerUrl}) {
        ...
        checkName('Provider', this.provider);
        checkName('Consumer', this.consumer);
    }

    static buildContractUri (provider, consumer, tag) {
        const arr = [provider, consumer];
        tag = process.env.CDC_TAG || tag;
        tag && arr.push(tag);
    ...
    initialize (contract) {
        ...
        this.tag = process.env.CDC_TAG || contract.tag;
        this.tag && checkName('Tag', this.tag);
