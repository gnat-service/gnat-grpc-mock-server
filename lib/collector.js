...
class Collector extends Picker {
    ...
    getFullContractUrl () {
        return Collector.getFullContractUrl(this.brokerUrl, this.provider, this.consumer, this.tag);
    }
    ...
    initialize (contract) {
        ...
        if (contract.brokerUrl) {
            this.brokerUrl = contract.brokerUrl.replace(/\/$/, '');
        }
...
class Consumer extends Collector {
    ...
    async _printFile (data) {
        ...
        const text = Consumer.getDataText(data);
    ...
    async _publishContract (data) {
        ...
        const url = this.getFullContractUrl();
        const body = Consumer.getDataText(data, 2);
...
class Provider extends Collector {
    ...
    static async getContractFromBroker ({brokerUrl, provider, consumer, tag}) {
        const url = Collector.getFullContractUrl(brokerUrl, provider, consumer, tag);
        return req.get({url});
    }
