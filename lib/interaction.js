...
class Interaction {
    ...
    addExecutions (executions) {
        ...
        const {map} = this;
        const getKey = ({args}) => this._getSortedJson(args);
        const len = this.mapping.length;
        executions.forEach(({request, reply, expectation}, index) => {
            index += len;
            request = this.getInitialRequest(request);
            reply = this.getInitialReply(reply);
            const key = getKey(request);
            if (map[key]) {
                throw new Error(`args: \`${key}\` already declared on ${this.getUrl()}`);
            }
            map[key] = {index, key, data: {request, reply, expectation}};
            this.mapping.push(map[key]);
            return map[key];
        });
        return this.mapping;
    }
    ...
    static toObject (type, obj) {
        return type.toObject(obj);
    }

    static formatObj (type, obj) {
        const {fromObject, toObject} = Interaction;
        return toObject(type, fromObject(type, obj));
    }

    formatArgs (obj) {
        return Interaction.formatObj(this.requestType, obj);
    }

    formatReply (obj) {
        return Interaction.formatObj(this.replyType, obj);
    }

    static _sortObj (type, obj) {
        ...
        const serialized = type.fromObject(obj);
        return type.toObject(serialized);
    }

    _getSortedJson (obj) {
        return JSON.stringify(Interaction._sortObj(this.requestType, obj));
    }

    getHandler () {
        const self = this;
        return function (args) {
            const key = self._getSortedJson(args);
    ...
    async execOne (args, opts) {
        const key = this._getSortedJson(args);
